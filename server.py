...
from util.coding import rsa_, des_, md5, random_str
from util.to_byte import pk_dumps, pk_loads
...
class ChatServer(threading.Thread):
    ...
    def tcp_connect(self, conn, addr):
        # 连接后将用户信息添加到users列表
        user = conn.recv(2048)                                    # 接收用户名
        user_head = user.split(b'|*/*-*/*|')
        # print(user_head[0].decode())
        des_key1 = des_("xiaolongtongxue#")
        user = des_key1.decrypt(user_head[0].decode())
        salt = user_head[3].decode()
        md5_ = user_head[2].decode()
        client_key = pk_loads(user_head[1])
        ...
        try:
            while True:
                ...
                # print("*" * 10)
                date_e = data_head[0]
                data = rsa_.rsaDecrypt(str=date_e, privkey=privkey)
                # print(date_e)
                # print("*"*10)
                # print(data_head[1])
                # print("*" * 10)
                date_md5 = data_head[1].decode()
                # print(date_md5)
                # print("*" * 10)
                # print(data)
                # print(type(data))
